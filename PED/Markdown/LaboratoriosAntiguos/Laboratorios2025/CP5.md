...
int main() {
    ...
    std::cout << "Elementos en la pila: " << pila.size() << std::endl; // → 4 elementos

    // Mostrar el elemento en la cima (peek / top)
    if (!pila.empty()) {
        std::cout << "Elemento en la cima (peek): " << pila.top() << std::endl; // → 2
    }

    // POP (desapilar elementos)
    std::cout << "\nDesapilando elementos..." << std::endl;
    while (!pila.empty()) {
        std::cout << "Sale: " << pila.top() << std::endl;
        pila.pop(); // elimina el elemento superior
    }

    // Verificar si está vacía
    if (pila.empty()) {
        std::cout << "\nLa pila ahora está vacía." << std::endl;
    }
...
```
```
Elementos en la pila: 4
Elemento en la cima (peek): 2
...
Puedes mirar un ejemplo completo de todas las funciones que se pueden hacer con las pilas, en este [Repositorio - stack library](https://github.com/UCASV/RecursosExtraPED/blob/main/PilasYColas/PilasLibreria.cc)
...
<p align="center">
  <img 
src="https://raw.githubusercontent.com/meaguilar/meaguilar.github.io/refs/heads/main/PED/Imagenes/CP5/amimacion-fila-unica.gif" 
    alt="Animación de fila única" 
    width="450" 
...
![Queue-data-structure](https://raw.githubusercontent.com/meaguilar/meaguilar.github.io/refs/heads/main/PED/Imagenes/CP5/Queue-data-structure.png)
...
Puedes mirar un ejemplo completo de todas las funciones que se pueden hacer con las colas, en este [Repositorio - manual queue](https://github.com/UCASV/RecursosExtraPED/blob/main/PilasYColas/ColasManual.cc)
...
int main() {
    // Declaramos una cola de enteros
    std::queue<int> cola;

    // ENQUEUE (agregar elementos al final)
    cola.push(1);
    cola.push(2);
    cola.push(3);
    cola.push(1);

    std::cout << "Elementos en la cola: " << cola.size() << std::endl; // → 4 elementos

    // Mostrar el primer elemento (peek / front)
    if (!cola.empty()) {
        std::cout << "Elemento al frente (peek): " << cola.front() << std::endl; // → 1
    }

    // DEQUEUE (eliminar el elemento del frente)
    std::cout << "\nDesencolando elementos..." << std::endl;
    while (!cola.empty()) {
        std::cout << "Sale: " << cola.front() << std::endl;
        cola.pop(); // elimina el frente
    }

    // Verificar si está vacía
    if (cola.empty()) {
        std::cout << "\nLa cola ahora está vacía." << std::endl;
    }
...
```
```
Elementos en la cola: 4
Elemento al frente (peek): 1
